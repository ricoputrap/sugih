...
} from "../../utils/series/incomeSeries";

export interface IncomeTrendsChartProps {
  /** Category income time series data */
...
/**
 * IncomeTrendsChart Component
 *
 * Displays income trends over time by category with multiple series.
...
export function IncomeTrendsChart({
  ...
  isLoading = false,
  title = "Income Trends",
  description = "Your income by category over time",
  className,
  maxCategories = 5,
}: IncomeTrendsChartProps) {
  // Get category names (sorted by total income)
  const categoryNames = React.useMemo(
    () => getIncomeCategoryNames(data).slice(0, maxCategories),
    [data, maxCategories],
  );
  ...
  const gradientDefs = React.useMemo(() => {
    ...
    return categoryNames.map((name, index) => {
      ...
      const gradientId = `gradient-income-${name.replace(/\s+/g, "")}`;
      return (
        <linearGradient
          key={gradientId}
          id={gradientId}
        ...
        <CardContent className="h-[350px] flex items-center justify-center">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-xs mt-1">
              Start recording income to see your income trends
            </p>
          </div>
        </CardContent>
      </Card>
