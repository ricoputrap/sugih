...
describe("SavingsBucket PostgreSQL Schema Validation", () => {
  ...
  describe("Zod Schema Validation", () => {
    describe("SavingsBucketCreateSchema", () => {
      ...
      it("should validate minimal creation data", () => {
        const result = SavingsBucketCreateSchema.safeParse(
          validSavingsBucketInputMinimal,
        );
        expect(result.success).toBe(true);
      });
    ...
    describe("BulkDeleteSavingsBucketsSchema", () => {
      it("should be defined", () => {
        expect(BulkDeleteSavingsBucketsSchema).toBeDefined();
      });

      it("should validate correct bulk delete data", () => {
        const result = BulkDeleteSavingsBucketsSchema.safeParse({
          ids: [
      ...
      it("should reject empty ids array", () => {
        ...
        if (!result.success) {
          expect(result.error.issues[0].message).toBe(
            "At least one savings bucket ID is required",
          );
        }
      });
      ...
      it("should accept up to 100 ids", () => {
        const result = BulkDeleteSavingsBucketsSchema.safeParse({
          ids: Array.from(
            { length: 100 },
            (_, i) => `bucket_${i.toString().padStart(10, "0")}`,
          ),
        });
        expect(result.success).toBe(true);
      });
      ...
      it("should reject more than 100 ids", () => {
        ...
        if (!result.success) {
          expect(result.error.issues[0].message).toBe(
            "Maximum 100 savings buckets can be deleted at once",
          );
        }
      });
  ...
  describe("Nanoid Handling", () => {
    it("should handle nanoid strings in id column", () => {
      const nanoid = "cPRN4GwjAn0EhLig1KJla";
      const result = SavingsBucketIdSchema.safeParse({ id: nanoid });
      expect(result.success).toBe(true);
    });

    it("should validate nanoid format", () => {
      const validNanoid = "cPRN4GwjAn0EhLig1KJla";
      const emptyId = "";

      const validResult = SavingsBucketIdSchema.safeParse({ id: validNanoid });
      const invalidResult = SavingsBucketIdSchema.safeParse({ id: emptyId });
...
And even more content.`;
  ...
  describe("Backward Compatibility", () => {
    ...
    it("should maintain same validation rules", () => {
      ...
      const result2 = SavingsBucketCreateSchema.safeParse({
        name: "Valid Name",
      });
