...
describe("GET /api/reports/spending-trend", () => {
  ...
  it("should return spending trend with default params", async () => {
    ...
    const request = new NextRequest("http://localhost/api/reports/spending-trend");
  ...
  it("should handle custom date range", async () => {
    const mockData = [{ period: "2024-01-01", totalAmount: 500000, transactionCount: 5 }];
    vi.mocked(spendingTrend).mockResolvedValue(mockData);

    const request = new NextRequest("http://localhost/api/reports/spending-trend?from=2024-01-01&to=2024-12-31&granularity=week");
    const response = await GET(request);
