...
import { createSavingsContribution } from "@/modules/Transaction/actions";
 ...
 * Create a new savings contribution transaction
 *
 * Body:
...
async function handlePost(request: NextRequest) {
  try {
    ...
    // Create the savings contribution transaction
    const transaction = await createSavingsContribution(body);
    ...
    return serverError("Failed to create savings contribution");
  }
}

export const POST = withRouteLogging(handlePost, {
  operation: "api.transactions.savings.contribute.create",
  logQuery: false,
