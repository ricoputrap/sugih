...
describe('Database Configuration', () => {
  ...
  describe('getDatabaseConfig', () => {
    it('should parse DATABASE_URL correctly', () => {
      process.env.DATABASE_URL = 'postgresql://user:pass@localhost:5432/sugih_dev';

      const config = getDatabaseConfig();

      expect(config.url).toBe('postgresql://user:pass@localhost:5432/sugih_dev');
    ...
    it('should handle DATABASE_URL with SSL', () => {
      process.env.DATABASE_URL = 'postgresql://user:pass@localhost:5432/sugih_dev?ssl=true';
    ...
    it('should throw error for invalid environment variables', () => {
      ...
      expect(() => getDatabaseConfig()).toThrow('DATABASE_URL must be a valid URL');
    });
  });
  ...
  describe('buildConnectionString', () => {
    it('should build connection string with all parameters', () => {
      ...
      expect(connectionString).toBe('postgresql://user:pass@localhost:5432/mydb?ssl=true');
    });
  ...
  describe('validateConfig', () => {
    ...
    it('should reject configuration without url or host/database', () => {
      ...
      expect(result.errors).toContain('Either url or (host and database) must be provided');
    });
    ...
    it('should reject low connection timeout', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Connection timeout should be at least 1000ms');
    });
  ...
  describe('getEnvironmentConfig', () => {
    it('should return environment information', () => {
      ...
      expect(envConfig.config.url).toBe('postgresql://user:pass@localhost:5432/sugih_dev');
    });
  ...
  describe('Configuration Persistence', () => {
    it('should maintain configuration state across multiple calls', () => {
      ...
      process.env.DATABASE_URL = 'postgresql://different:config@localhost:5432/sugih_dev';

      const config2 = getDatabaseConfig();

      // Should return the same cached configuration
      expect(config1).toBe(config2);
      expect(config1.url).toBe('postgresql://user:pass@localhost:5432/sugih_dev');
    });
  });
});
